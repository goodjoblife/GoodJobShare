...
export const validReason = isLimit =>
  R.anyPass([
    () => !isLimit,
    R.compose(R.allPass([n => n > 0, n => n <= 500]), R.length),
  ]);
...
export const validReasonCategory = reasonCategory =>
  R.any(
    options =>
      R.contains(
        reasonCategory,
        options.map(r => r.value),
      ),
    [experienceReportReasons, salaryReportReasons],
  );
...
export const validReasomForm = R.allPass([
  ...
  R.compose(validReasonCategory, R.prop('reasonCategory')),
]);
