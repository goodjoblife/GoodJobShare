...
const pageSelector = state => state.campaignTimeAndSalaryBoard.get('currentPage');
...
const setBoardData = ({ campaignName, sortBy, order }, { status, data, total = 0, currentPage = 0, error = null }) =>
  (dispatch, getState) => {
...
export const queryCampaignTimeAndSalary = (campaignName, { sortBy, order, jobTitles, page }) =>
  (dispatch, getState) => {
    if (
      campaignName !== campaignNameSelector(getState()) ||
      sortBy !== sortBySelector(getState()) ||
    ...
    return fetchCampaignTimeAndSalary(campaignName, opt)
      .then(rawData => {
        ...
        dispatch(setBoardData({ campaignName, sortBy, order }, {
