import { SITE_NAME, IMG_HOST } from '../constants/helmetConstants';
...
export class HelmetData {
  constructor(pageName) {
    const _HELMET_DATA = {
      DEFAULT: {
        ...
        meta: [
          ...
          { property: 'og:image', content: `${IMG_HOST}/og/default.jpg` },
      ...
      LANDING_PAGE: {
        meta: [
          { property: 'og:image', content: `${IMG_HOST}/og/index.jpg` },
        ],
      },
      SHARE: {
        title: '分享你的職場資訊',
        meta: [
          ...
          { property: 'og:image', content: `${IMG_HOST}/og/share.jpg` },
        ],
        link: [
      ...
      SHARE_INTERVIEW: {
        ...
        meta: [
          ...
          { property: 'og:image', content: `${IMG_HOST}/og/share-interview.jpg` },
        ],
        link: [
      ...
      SHARE_WORK: {
        ...
        meta: [
          ...
          { property: 'og:image', content: `${IMG_HOST}/og/share-work.jpg` },
        ],
        link: [
      ...
      EXPERIENCE_SEARCH: {
        ...
        meta: [
          ...
          { property: 'og:image', content: `${IMG_HOST}/og/experience-search.jpg` },
        ],
        link: [
      ...
      LABOR_RIGHTS_MENU: {
        ...
        meta: [
          ...
          { property: 'og:image', content: `${IMG_HOST}/og/labor-rights.jpg` },
      ...
      FAQ: {
        ...
        meta: [
          ...
          { property: 'og:image', content: `${IMG_HOST}/og/faq.jpg` },
        ],
        link: [
    ...
    this.data = _HELMET_DATA[pageName] || {};
  }
  ...
  setValue(key, value) {
    ...
    } else if (metaNameList.indexOf(key) >= 0) { // for head meta (description, keywords ... )
      if (this.data.meta) {
        const obj = this.data.meta.find(e => e.name === key);
        if (obj) {
          obj.content = value;
        } else {
          this.data.meta.push({ name: key, content: value });
        }
      } else {
        this.data.meta = [{ name: key, content: value }];
      }
    } else if (metaPropertyList.indexOf(key) >= 0) { // for open graph protocol (og:xxx)
      if (this.data.meta) {
        const obj = this.data.meta.find(e => e.property === key);
        if (obj) {
          obj.content = value;
        } else {
          this.data.meta.push({ property: key, content: value });
        }
      } else {
        this.data.meta = [{ property: key, content: value }];
      }
    } else if (key === 'link') { // for link
      if (this.data.link) {
        const obj = this.data.link.find(e => e.rel === 'canonical');
        if (obj) {
          obj.href = value;
        } else {
          this.data.link.push({ rel: 'canonical', href: value });
        }
      } else {
        this.data.link = [{ rel: 'canonical', href: value }];
      }
    }
    return this;
  }
